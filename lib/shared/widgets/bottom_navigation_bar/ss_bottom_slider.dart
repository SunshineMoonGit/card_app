...
class SsBottomSlider extends StatefulWidget {
  ...
  SsBottomSliderController? controller;

  /// This object used to control additional animation  for footer
  SsBottomSliderController? footerController;

  /// SsBottomSlider Constructor
  SsBottomSlider({
    super.key,
    this.footer,
    this.appBar,
    ...
    if (controller == null) {
      ...
      this.controller = SsBottomSliderController();
    }
    if (footerController == null) {
      // ignore: unnecessary_this
      this.footerController = SsBottomSliderController(initial: true);
    }
  }
...
class _SsBottomSliderState extends State<SsBottomSlider> with TickerProviderStateMixin {
  ...
  SsBottomSliderController get _effectiveController => widget.controller!;
  SsBottomSliderController get _effectiveFooterController => widget.footerController!;
  ...
  void initState() {
    ...
    _effectiveController.addListener(pushRoute);
    // Super Init State
    super.initState();
  }

  void pushRoute() {
    if (_effectiveController.isOpened) {
      _ac.value = 0;
      _effectiveController.hide();
      // context.push(MyCardScreen.route);
    }
  }

  /// Required for resubscribing when hot reload occurs [ValueNotifier]
  @override
  void didUpdateWidget(SsBottomSlider oldWidget) {
